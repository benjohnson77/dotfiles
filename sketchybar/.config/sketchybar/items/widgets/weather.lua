...
local function fetch_weather_data(callback)
	local command = "$CONFIG_DIR/items/widgets/akt_weather.sh"
...
local function toggle_details()
	...
	if should_draw then
		...
		sbar.exec("$CONFIG_DIR/items/widgets/akt_weather.sh", function(result)
			...
			windspeed:set({ label = popup_wind_speed .. " mph" })
			cloudcover:set({ label = popup_cloud_cover .. " %" })
			local visibility_miles = tonumber(popup_visibility)
			visibility:set({ label = visibility_miles .. " mi" })
		end)
	else
		hide_details()
	end
